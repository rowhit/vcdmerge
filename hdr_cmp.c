...
static int compare_vars(struct vcd_var *v_a,struct vcd_var *v_b)
{
	int rv=0;
	scmp_s(v,tp);
//	scmp_s(v,id);
...
static struct vcd_var *find_same_var(struct vcd_scope *scp,struct vcd_var *v_a)
{
	size_t i;
...
static int compare_nvars(struct vcd_scope *scp_a,struct vcd_scope *scp_b)
{
	int rv=0;
	size_t i;
...
static struct vcd_scope *find_same_scope(struct vcd_scope *scp,struct vcd_scope *cscp_a)
{
	size_t i;
	int rv=0;
...
static int compare_nscopes(struct vcd_scope *scp_a,struct vcd_scope *scp_b)
{
	int rv=0;
	size_t i;
...
int vcdCompareHeaders(struct vcd_hdr *hdr_a,struct vcd_hdr *hdr_b)
{
	int rv;
