...
static int proc_date(void *d,FILE *fp)
{
	return skip_to_end(fp);
}

static int proc_ver(void *d,FILE *fp)
{
	return skip_to_end(fp);
}

static int proc_ts(void *d,FILE *fp)
{
...
static int proc_scope(void *d,FILE *fp)
{
	...
	scp->n_scopes++;
	scp->scopes=realloc(scp->scopes,scp->n_scopes*sizeof(struct vcd_scope));
	assert(scp->scopes);
	...
	newscope->prev=scp;
	do{}while(SEEN_UPSCOPE!=process_tag(newscope,fp));
	return 0;
}

static int proc_var(void * d,FILE *fp)
{
	struct vcd_scope *scp=d;
	char *svp=NULL;
...
static int proc_upscope(void * d,FILE *fp)
{
...
static int proc_end(void * d,FILE *fp)
{
	assert(0);//unhandled
}

static int proc_defend(void * d,FILE *fp)
{
	skip_to_end(fp);
	return SEEN_ENDDEF;
}
...
int vcdReadHeader(struct vcd_hdr *hdr,FILE *fp)
{
	...
	do{}while(SEEN_ENDDEF!=process_tag(&hdr->base,fp));
	return 0;
}

static void clear_var(struct vcd_var*v)
{
	v->prev=NULL;
	free(v->tp);
	v->tp=NULL;
...
static void clear_scope(struct vcd_scope *scp)
{
	size_t i;
